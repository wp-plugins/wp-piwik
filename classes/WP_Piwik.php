...
class WP_Piwik {
	...
	private function addJavascriptCode() {
		...
		$trackingCode = new WP_Piwik_TrackingCode($this->subClassConfig());
	...
	public function addPostMetaboxes() {
		if (self::$settings->getGlobalOption('add_customvars_box')) {
			add_action('add_meta_boxes', array(new WP_Piwik_Template_MetaBoxCustomVars($this->subClassConfig()), 'addMetabox'));
			add_action('save_post', array(new WP_Piwik_Template_MetaBoxCustomVars($this->subClassConfig()), 'saveCustomVars'), 10, 2);
		}
		if (self::$settings->getGlobalOption('perpost_stats')) {
			add_action('add_meta_boxes', array(new WP_Piwik_Template_MetaBoxPerPostStats($this->subClassConfig()), 'addMetabox'));
		}
	}
	...
	function setPluginMeta($strLinks, $strFile) {
		...
		if ($strFile == self::$strPluginBasename) 
			return array_merge(
				$strLinks,
				array(
					sprintf('<a href="'.$this->getSettingsURL().'">%s</a>', __('Settings', 'wp-piwik'))
				)
			);
	...
	function callPHP($strParams) {
		...
		if (file_exists(PIWIK_INCLUDE_PATH.DIRECTORY_SEPARATOR.'index.php'))
			require_once PIWIK_INCLUDE_PATH.DIRECTORY_SEPARATOR.'index.php';
		if (file_exists(PIWIK_INCLUDE_PATH.DIRECTORY_SEPARATOR.'core'.DIRECTORY_SEPARATOR.'API'.DIRECTORY_SEPARATOR.'Request.php'))
			require_once PIWIK_INCLUDE_PATH.DIRECTORY_SEPARATOR.'core'.DIRECTORY_SEPARATOR.'API'.DIRECTORY_SEPARATOR.'Request.php';
	...
	function createDashboardWidget($objPost, $aryMetabox) {
		...
		// Include widget file
		if (file_exists(WP_PIWIK_PATH.'dashboard'.DIRECTORY_SEPARATOR.$strFile.'.php'))
			include(WP_PIWIK_PATH.'dashboard'.DIRECTORY_SEPARATOR.$strFile.'.php');
 	}
...
<?php /***************************************************************************/
	...
	private static function showErrorMessage($strMessage) {
		echo '<strong class="wp-piwik-error">'.__('An error occured', 'wp-piwik').':</strong> '.$strMessage.' [<a href="'.$this->getSettingsURL().'&tab=support">'.__('Support','wp-piwik').'</a>]';
	}
