...
class WP_Piwik {
	 ...
	 *
	 * @param int $postID The new post's ID
	 */
	public function addPiwikAnnotation($postID) {
		$note = 'Published: ' . get_post($postID)->post_title . ' - URL: ' . get_permalink($postID);
		$id = WP_Piwik\Request::register('Annotations.add', array('idSite' => $this->getPiwikSiteId(), 'date' => date('Y-m-d'), 'note' => $note));
	 ...
	 * @param strint $oldStatus new post status
	 ...
	 *
	 * @param array full list of column settings
	 * @param mixed current screen id
	...
	public function shortcode($attributes) {
		shortcode_atts(array('title' => '', 'module' => 'overview', 'period' => 'day', 'date' => 'yesterday', 'limit' => 10, 'width' => '100%', 'height' => '200px', 'language' => 'en', 'range' => false, 'key' => 'sum_daily_nb_uniq_visitors'), $attributes);
	 ...
	 *
	 * @param int $blogId which blog's Piwik site ID to get, default is the current blog
	 ...
	 *
	 * @param int $blogId which blog's Piwik site ID to estimate, default is the current blog
	 * @return mixed Piwik site ID or n/a
	 */
	private function requestPiwikSiteId($blogId = null) {
		...
		if (self::$settings->getGlobalOption('auto_site_config')) {
			$id = WP_Piwik\Request::register('SitesManager.getSitesIdFromSiteUrl', array('url' => $isCurrent ? get_bloginfo('url') : get_blog_details($blogId)->siteurl));
			$result = $this->request($id);
	 ...
	 * Add a new Piwik
	 *
	 * @param int $blogId which blog's Piwik site to create, default is the current blog
	 * @return int Piwik site ID
	 */
	public function addPiwikSite($blogId = null) {
		$isCurrent = !self::$settings->checkNetworkActivation() || empty($blogId);
		$id = WP_Piwik\Request::register('SitesManager.addSite', array('urls' => $isCurrent ? get_bloginfo('url') : get_blog_details($blogId)->$siteurl, 'siteName' => $isCurrent ? get_bloginfo('name') : get_blog_details($blogId)->$blogname));
		$result = $this->request($id);
		self::$logger->log('Get Piwik ID: WordPress site ' . ($isCurrent ? get_bloginfo('url') : get_blog_details($blogId)->$siteurl) . ' = Piwik ID ' . $result);
		if (empty($result) || !isset($result[0]))
			return null;
	 ...
	 *
	 * @param int $siteId which Piwik site to updated
	 * @param int $blogId which blog's Piwik site ID to get, default is the current blog
	 */
	private function updatePiwikSite($siteId, $blogId = null) {
		$isCurrent = !self::$settings->checkNetworkActivation() || empty($blogId);
		$id = WP_Piwik\Request::register('SitesManager.updateSite', array('idSite' => $siteId, 'urls' => $isCurrent ? get_bloginfo('url') : get_blog_details($blogId)->$siteurl, 'siteName' => $isCurrent ? get_bloginfo('name') : get_blog_details($blogId)->$blogname));
		$result = $this->request($id);
	 ...
	 *
	 * @param string $oldValue old blog name
	 * @param string $newValue new blog name
	 */
	 ...
	 *
	 * @param string $oldValue old blog URL
	 * @param string $newValue new blog URL
	 */
	 ...
	 *
	 * @param mixed $statsPageId WordPress stats page ID
	 */
	 ...
	 *
	 * @param mixed $postPageId WordPress post page ID
	 */
